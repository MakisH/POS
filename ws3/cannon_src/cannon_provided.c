...
int main (int argc, char **argv) {
	...
	// Makis: There is no point in making this communication non-blocking. Each process needs the matrices_a_b_dimensions
	//	  immediately after receiving. Also, since a process receives, it continues without taking part in any other comm.
	//	  I am not sure whether a broadcast would change the performance behavior.
	if(rank == 0) {
		int i;
		for(i = 1; i < size; i++){
	...
	for(cannon_block_cycle = 0; cannon_block_cycle < sqrt_size; cannon_block_cycle++){
		...
		start = MPI_Wtime();
		// Makis: Replace this with:
		//	* allocate send buffers for A and B
		//	* MPI_Wait(sendA)
		//	* fill the send buffer for A
		// 	* MPI_Isend request for the send buffer of A
		//	* MPI_Irecv request for the actual block of A
		//	* MPI_Wait(sendB)
		//	* fill the send buffer for B
		//	* MPI_Isend request for the send buffer of B
		//	* MPI_Irecv request for the actual block of B
		//	* MPI_Wait(recv) before the for(C_index) (for both A and B)
		//	* do some useful work...
		//	Note: we may spend time to fill buffers, but probably we will 
		//	gain this time and more by overlapping communication with computations.	
