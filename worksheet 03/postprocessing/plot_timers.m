...
xlabel('Size N of the NxN input matrices')
ylabel('Time (s)')
title('Computation (green) and MPI (red) times (One-sided case - Sandybridge)')

% 
% %%%%%% One-sided case - Haswell %%%%%%
% 
VERSION = 'onesided';
ARCH = 'hw';
N=64;
...
xlabel('Size N of the NxN input matrices')
ylabel('Time (s)')
title('Computation (green) and MPI (red) times (One-sided case - Haswell)')
...
for i=1:N_COUNT
   ...
   % 6 - Speedup of comp_time for the one-sided case, Sandybridge
   speedup(i,6) = stat_onesided_sb(i,4);
   % 7 - Speedup of comp_time for the one-sided case, Haswell
   speedup(i,7) = stat_onesided_hw(i,4);
   % 8 - Speedup of mpi_time for the one-sided case, Sandybridge
   speedup(i,8) = stat_onesided_sb(i,5);
   % 9 - Speedup of mpi_time for the one-sided case, Haswell
   speedup(i,9) = stat_onesided_hw(i,5);
   N = N*2;
end
...
figure
subplot(1,2,1)
plot(speedup(:,1), speedup(:,6), 'ro');
hold on
plot(speedup(:,1), speedup(:,7), 'b*');
plot([0, 5060], [1, 1], 'k--');
hold off
...
xlabel('Size N of the NxN input matrices')
ylabel('Speedup [comp\_time]')
title('Speedup [comp\_time] (median) - One-sided')
legend('SuperMUC Phase I - Sandybridge', 'SuperMUC Phase II - Haswell', 'Location','northwest')
...
xlabel('Size N of the NxN input matrices')
ylabel('Speedup [mpi\_time]')
title('Speedup [mpi\_time] (median) - One-sided')
legend('SuperMUC Phase I - Sandybridge', 'SuperMUC Phase II - Haswell', 'Location','northwest')
